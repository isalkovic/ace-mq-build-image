...
func createWorkDir() error {
	...
	if err != nil {
		...
		if qmgr.UseQueueManager() {
		// changing command to test with Openshift	_, _, err := command.RunAsUser("1000", "/opt/ibm/ace-12/server/bin/mqsicreateworkdir", "/home/aceuser/ace-server")
		//	_, _, err := command.RunAsUser("1000", "/opt/ibm/ace-12/server/bin/mqsicreateworkdir", "/home/aceuser/ace-server")
			cmd := exec.Command("/opt/ibm/ace-12/server/bin/mqsicreateworkdir", "/home/aceuser/ace-server")
			_, _, err := command.RunCmd(cmd)
			// end added lines (2 above)
			if err != nil {
